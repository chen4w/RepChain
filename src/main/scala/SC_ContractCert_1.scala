 ...
 * @author zyf
 */
final case class SC_ContractCert_1CertStatus(credit_code: String, name: String, status: Boolean)
final case class SC_ContractCert_1CertInfo(credit_code: String, name: String, cert: Certificate)
class SC_ContractCert_1 extends IContract{
  //case class SC_ContractCert_1CertStatus(credit_code: String, name: String, status: Boolean)
  //case class SC_ContractCert_1CertInfo(credit_code: String, name: String, cert: Certificate)
   ...
   * @return
   */
  def signUpCert(ctx: ContractContext, data:SC_ContractCert_1CertInfo): ActionResult = {
    val isNodeCert = ctx.api.bNodeCreditCode(ctx.t.getSignature.getCertId.creditCode)
    if (!isNodeCert) {
      throw ContractException(notNodeCert)
    }
   ...
   * @return
   */
  def updateCertStatus(ctx: ContractContext, data:SC_ContractCert_1CertStatus): ActionResult = {
    val isNodeCert = ctx.api.bNodeCreditCode(ctx.t.getSignature.getCertId.creditCode)
    if (!isNodeCert) {
      throw ContractException(notNodeCert)
    }
  ...
  override def onAction(ctx: ContractContext, action: String, sdata: String): ActionResult = {
    ...
    action match {
      ...
      case ACTION.SignUpCert =>
        println("SignUpCert")
        signUpCert(ctx, json.extract[SC_ContractCert_1CertInfo])
      case ACTION.UpdateCertStatus =>
        println("UpdateCertStatus")
        updateCertStatus(ctx, json.extract[SC_ContractCert_1CertStatus])
